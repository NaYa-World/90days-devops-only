...
function updateStats() {
  ...
  const freezeAvail=document.getElementById('freeze-avail');
  const avail=getFreezesAvail();
  if(freezeBtn){ freezeBtn.disabled=(avail===0||streak===0); }
  if(freezeAvail){ freezeAvail.textContent=avail>0?'(1 available)':'(used this week)'; }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(btn) btn.addEventListener('click', function(){
    if(getFreezesAvail()>0){ useFreeze(); updateStats(); showToast('❄ Streak freeze used! Your streak is protected for yesterday.'); }
  });
  // Smart next click
